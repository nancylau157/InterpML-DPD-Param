...
data = np.loadtxt('DatasetL64_Agg.txt', delimiter='\t')
...
Ls=[1,1]
kernel = C(1, (1e-5, 1e-3)) * RBF(Ls, (0.000001, 2000000))
...
x1 = np.linspace(0, 1, 37)
x2 = np.linspace(0, 1, 37)
...
print("Predicted Agg:",y_test_pred)
print("Actual Agg:",y_test)
...
plt.fill_between(X_pred[:, 1],
                 y_pred - 1.9600 * sigma,
                 y_pred + 1.9600 * sigma,
                 alpha=0.5, color='g', label='$95\%$ confidence interval')
plt.xlabel(r'$a_{BW}$', fontsize=18, color='black')
plt.ylabel('$A_s$', fontsize=18, color='black')
plt.ylim(-500, 900)
#plt.title(f'Prediction with {gp.kernel_}', fontsize=16)
plt.legend(loc='upper left', fontsize=16)
plt.grid(True)
plt.savefig('aBW_Agg.png', dpi=300)
plt.show()
...
plt.ylabel(r'$A_s$', fontsize=18, color='black')
plt.ylim(-500, 900)
#plt.title(f'Prediction with {gp.kernel_}', fontsize=16)
plt.legend(loc='upper left', fontsize=16)
plt.grid(True)
plt.savefig('aAW_Agg.png', dpi=300)
plt.show()
...
ax1.set_zlabel("$\mathbf{Agg}$", fontweight="bold", fontsize=12, color='black')
ax1.view_init(elev=30, azim=220)
cbar1 = fig3.colorbar(surf1, ax=ax1, shrink=0.6, aspect=10)
cbar1.set_label('Predicted Agg', fontsize=18)
...
plt.savefig('GPR_Agg_bivariate_subplot1.png', dpi=300)
plt.show()
...
ax2.set_ylabel(r'$a_{BW}$', fontweight="bold", fontsize=30, color='black',labelpad=20)
ax2.set_zlabel(r'$A_s$', fontweight="bold", fontsize=30, color='black',labelpad=18)
...
cbar2.set_label(r'Predicted $A_s$', fontsize=30, labelpad=10)
cbar2.set_ticks(np.linspace(np.min(z_data), np.max(z_data), 8))  
cbar2.ax.yaxis.set_major_formatter(plt.FormatStrFormatter('%3d'))
...
plt.savefig('GPR_AGG_bivariate_subplot2.png', dpi=300)
plt.show()
...
#plt.title(r'$A_s$ actual values')
plt.ylabel(r'$A_s$ predicted values', color='black', fontsize=30)
plt.xlabel(r'$A_s$ actual values', color='black',fontsize=30)
