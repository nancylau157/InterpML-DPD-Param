...
data = np.loadtxt('DatasetL64_CMC.txt', delimiter='\t')
...
Ls=[1,1]
kernel = C(1, (1e-3, 1e-3)) * RBF(Ls, (0.00001, 200000))
...
# Sorting the matrix by the first column, then by the second column
sorted_matrix = np.sort(matrix, axis=0)

sorted_matrix[:, 1] = 0.42857143 
print(matrix[:,1])
...
print("Predicted CMC:",y_test_pred)
print("Actual CMC:",y_test)
...
plt.xlabel('$a_{BW}$', fontsize=18, color='black')
plt.ylabel('CMC', fontsize=18, color='black')
plt.ylim(-0.01, 0.07)
#plt.title(r'Prediction with {gp.kernel_}', fontsize=16)
plt.legend(loc='upper left', fontsize=16)
plt.grid(True)
plt.savefig('aBW_CMC.png', dpi=300)
plt.show()
...
plt.ylabel(r'CMC', fontsize=18, color='black')
plt.ylim(-0.01, 0.07)
#plt.title(r'Prediction with {gp.kernel_}', fontsize=16)
plt.legend(loc='upper left', fontsize=16)
plt.grid(True)
plt.savefig('aAW_CMC.png', dpi=300)
plt.show()
...
ax1.scatter(X_train[:, 0], X_train[:, 1], y_train, c='red', s=100, marker="o", edgecolors='r', linewidths=1.5, label=r'Training points')
ax1.scatter(X_test[:, 0], X_test[:, 1], y_test,  c='blue', s=100, marker="^", edgecolors='b', linewidths=1.5, label=r'Validation points')
surf1 = ax1.plot_surface(x1_data, x2_data, z_data, cmap=cmap, edgecolor='none', alpha=0.8)
ax1.set_xlabel(r'$a_{AW}$', fontweight="bold", fontsize=16, color='black')
ax1.set_ylabel(r'$a_{BW}$', fontweight="bold", fontsize=16, color='black')
ax1.set_zlabel(r'CMC', fontweight="bold", fontsize=16, color='black')
ax1.view_init(elev=30, azim=220)
cbar1 = fig3.colorbar(surf1, ax=ax1, shrink=0.6, aspect=10)
cbar1.set_label(r'Predicted CMC', fontsize=18)
...
ax1.legend([r'Training points', r'Validation points'], loc='upper right', fontsize=16)
plt.savefig('GPR_CMC_bivariate_subplot1.png', dpi=300)
plt.show()
...
ax2.set_zlabel(r'CMC', fontweight="bold", fontsize=30, color='black',labelpad=22)
...
cbar2.set_label(r'Predicted CMC', fontsize=30)
cbar2.set_ticks(np.linspace(np.min(z_data), np.max(z_data), 8))  
cbar2.ax.yaxis.set_major_formatter(plt.FormatStrFormatter('%.3f'))
ax2.legend([r'Training points', r'Validation points'], loc='upper left', fontsize=28)
fig4.subplots_adjust(left=0, right=0.96, top=0.99, bottom=0.)
plt.savefig('GPR_CMC_bivariate_subplot2.png', dpi=300)
plt.show()

plt.figure(5)
fig5=plt.figure(figsize=(9, 6))
plt.scatter(y_test, y_test_pred, color='blue', s=50, marker="^", edgecolors='b', label=r'Predicted')
plt.plot(y_test, y_test, color='red', label=r'Ideal line')
#plt.title(r'CMC actual values')
plt.ylabel(r'CMC predicted values', color='black', fontsize=30)
plt.xlabel(r'CMC actual values', color='black',fontsize=30)
plt.text(0.1, 0.6, fr'R-squared: {r2_validation:.2f}', transform=plt.gca().transAxes, color='green', fontsize=26)
plt.tick_params(axis='both', which='major', labelsize=24)  # major ticks
